...
public class DynamicConfigUpdater {
    ...
    private final ConcurrentHashMap<String, String> configVersions = new ConcurrentHashMap<>();
    ...
    public void init() {
        ...
        // 监听日志配置变化
        registerConfigListener("logging-appender", "basebackend", this::updateLoggingConfig);

        // 监听统计配置变化
        registerConfigListener("statistics-config", "basebackend", this::updateStatisticsConfig);

        // 监听缓存配置变化
        registerConfigListener("cache-config", "basebackend", this::updateCacheConfig);

        // 监听监控配置变化
        registerConfigListener("monitoring-config", "basebackend", this::updateMonitoringConfig);
     ...
     */
    public void registerConfigListener(String dataId, String group, ConfigChangeHandler handler) {
     ...
     * @param handler 配置变更处理器
     */
    private void loadAndUpdateConfig(String dataId, String group, ConfigChangeHandler handler) {
        try {
     ...
     * @param handler 配置变更处理器
     */
    private void handleConfigChange(String dataId, String group, ConfigChangeHandler handler) {
        try {
            String content = nacosConfigManager.getConfig(dataId, group);
            if (content != null) {
                if (hasConfigChanged(dataId, group, content)) {
