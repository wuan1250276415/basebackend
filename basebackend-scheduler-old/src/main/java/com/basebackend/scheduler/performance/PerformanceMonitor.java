...
public class PerformanceMonitor {
    ...
    public void recordDatabaseQueryTime(String queryName, long queryTime) {
        ...
        customMetrics.asMap().merge(metricName, queryTime, Long::sum);
    ...
    public void recordCacheOperation(String cacheName, boolean hit) {
        ...
        customMetrics.asMap().merge(hitMetricName, hit ? 1L : 0L, Long::sum);
        customMetrics.asMap().merge(missMetricName, hit ? 0L : 1L, Long::sum);

        // 计算命中率
        Long hitsObj = customMetrics.getIfPresent(hitMetricName);
        Long missesObj = customMetrics.getIfPresent(missMetricName);
        long hits = hitsObj != null ? hitsObj : 0L;
        long misses = missesObj != null ? missesObj : 0L;
