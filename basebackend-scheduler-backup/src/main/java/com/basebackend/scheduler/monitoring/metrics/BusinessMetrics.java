...
public class BusinessMetrics {
    ...
    public double getTaskSuccessRate() {
        long succeeded = taskSucceededCounter.count();
        long failed = taskFailedCounter.count();
        long total = succeeded + failed;
        return total > 0 ? (double) succeeded / total * 100 : 0;
    }
    ...
    public Map<String, Object> getWorkflowTypeStats(String workflowType) {
        ...
        stats.put("started", started != null ? started.get() : 0L);
        stats.put("completed", completed != null ? completed.get() : 0L);
        stats.put("failed", failed != null ? failed.get() : 0L);

        long total = stats.get("started") == null ? 0L : (Long) stats.get("completed") + (Long) stats.get("failed");
        stats.put("successRate", total > 0 ? (Double) stats.get("completed") / total * 100 : 0);
