...
class ProcessInstanceServiceImplTest {
    ...
    /**
     * 设置 ProcessInstanceQuery 的 Mock
     */
    ...
    /**
     * 设置 HistoricProcessInstanceQuery 的 Mock
     */
    ...
    /**
     * 设置 VariableInstanceQuery 的 Mock
     */
    private void setupVariableInstanceQuery() {
        ...
        // ✅ 添加 singleResult() 的设置 - 修复 variable() 方法测试
    ...
    void testMigrate_Success() {
        ...
        // ✅ 修复：创建migrationInstructionsBuilder mock
        org.camunda.bpm.engine.migration.MigrationInstructionsBuilder migrationInstructionsBuilder =
            mock(org.camunda.bpm.engine.migration.MigrationInstructionsBuilder.class);

        when(runtimeService.createMigrationPlan(eq(sourceDefinitionId), eq("new-definition-id")))
                .thenReturn(migrationPlanBuilder);
        // ✅ 修复：mapEqualActivities()返回MigrationInstructionsBuilder
        when(migrationPlanBuilder.mapEqualActivities()).thenReturn(migrationInstructionsBuilder);
        when(migrationInstructionsBuilder.build()).thenReturn(migrationPlan);
    ...
    void testHistory_Success() {
        ...
        when(historic1.getStartTime()).thenReturn(new Date());
        when(historic1.getEndTime()).thenReturn(new Date());
        ...
        when(historic1.getState()).thenReturn("COMPLETED");
        ...
        when(historic2.getStartTime()).thenReturn(new Date());
        when(historic2.getEndTime()).thenReturn(new Date());
        ...
        when(historic2.getState()).thenReturn("COMPLETED");
