...
observability:
  trace:
    endpoint: http://localhost:3200  # Tempo endpoint
...
observability:
  trace:
    endpoint: http://localhost:9411  # Zipkin endpoint
    format: zipkin
    enabled: true
```
...
```bash
# Tempo
export TRACE_ENDPOINT=http://localhost:3200
export TRACE_FORMAT=tempo

# Zipkin
export TRACE_ENDPOINT=http://localhost:9411
...
```
DEBUG c.b.o.s.i.TraceQueryServiceImpl - Querying traces: http://localhost:9411/api/v2/traces?serviceName=basebackend-user-api&limit=100
```
