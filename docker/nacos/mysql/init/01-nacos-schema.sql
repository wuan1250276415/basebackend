...
CREATE TABLE IF NOT EXISTS `health_check` (
    ...
    `gmt_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_ip_port_cluster` (`ip`,`port`,`cluster_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='health_check';

-- 添加服务健康检查表
CREATE TABLE IF NOT EXISTS `service_health_check` (
    `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'id',
    `service_name` varchar(100) NOT NULL COMMENT 'service_name',
    `ip` varchar(50) NOT NULL COMMENT 'ip',
    `port` int(11) NOT NULL COMMENT 'port',
    `cluster_name` varchar(100) NOT NULL COMMENT 'cluster_name',
    `healthy` tinyint(1) NOT NULL DEFAULT '1' COMMENT 'healthy',
    `last_heartbeat` bigint(20) NOT NULL COMMENT 'last_heartbeat',
    `gmt_create` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `gmt_modified` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_service_ip_port_cluster` (`service_name`,`ip`,`port`,`cluster_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='service_health_check';
