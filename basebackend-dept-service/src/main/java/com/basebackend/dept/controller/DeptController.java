package com.basebackend.dept.controller;

import com.basebackend.common.model.Result;
import com.basebackend.dept.dto.DeptDTO;
import com.basebackend.dept.service.DeptService;
...
@Validated
@Tag(name = "部门管理", description = "部门组织架构管理相关接口")
public class DeptController {
     ...
     */
    @GetMapping
    @Operation(summary = "获取部门列表", description = "获取所有部门的平铺列表")
     ...
     */
    @GetMapping("/{id}")
    @Operation(summary = "根据ID查询部门", description = "根据部门ID查询部门详情")
     ...
     */
    @DeleteMapping("/{id}")
    @Operation(summary = "删除部门", description = "删除部门（如果存在子部门则不允许删除）")
    ...
    @Operation(summary = "获取子部门", description = "根据部门ID获取直接子部门列表")
    public Result<List<DeptDTO>> getChildrenByDeptId(@Parameter(description = "部门ID") @PathVariable Long id) {
        ...
        try {
            ...
            log.error("根据部门ID获取子部门列表失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
    ...
    @Operation(summary = "获取子部门ID列表", description = "根据部门ID获取所有子部门ID列表（递归查询）")
    public Result<List<Long>> getChildrenDeptIds(@Parameter(description = "部门ID") @PathVariable Long id) {
        ...
        try {
            ...
            log.error("根据部门ID获取所有子部门ID列表失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
    ...
    public Result<Boolean> checkDeptNameUnique(
            ...
            @Parameter(description = "部门ID（更新时需要传）") @RequestParam(required = false) Long deptId) {
        try {
            boolean unique = deptService.checkDeptNameUnique(deptName, parentId, deptId);
            return Result.success("检查完成", unique);
        } catch (Exception e) {
            log.error("检查部门名称唯一性失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @GetMapping("/by-name")
    @Operation(summary = "根据部门名称查询", description = "根据部门名称查询部门信息（用于服务间调用）")
    public Result<DeptDTO> getByDeptName(@Parameter(description = "部门名称") @RequestParam String deptName) {
        ...
        try {
            ...
            log.error("根据部门名称查询失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @GetMapping("/by-code")
    @Operation(summary = "根据部门编码查询", description = "根据部门编码查询部门信息（暂未实现）")
    public Result<DeptDTO> getByDeptCode(@Parameter(description = "部门编码") @RequestParam String deptCode) {
        ...
        try {
            ...
            log.warn("根据部门编码查询功能暂未实现: {}", e.getMessage());
            return Result.error(e.getMessage());
        } catch (Exception e) {
            log.error("根据部门编码查询失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @GetMapping("/batch")
    @Operation(summary = "批量查询部门", description = "根据部门ID列表批量查询部门信息（用于服务间调用）")
    public Result<List<DeptDTO>> getBatchByIds(@Parameter(description = "部门ID列表（逗号分隔）") @RequestParam String deptIds) {
        ...
        try {
            ...
            log.error("批量查询部门失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
    ...
    @Operation(summary = "根据父部门ID查询", description = "根据父部门ID获取直接子部门列表（用于服务间调用）")
    public Result<List<DeptDTO>> getByParentId(@Parameter(description = "父部门ID") @RequestParam Long parentId) {
        ...
        try {
            ...
            log.error("根据父部门ID查询失败: {}", e.getMessage(), e);
            return Result.error(e.getMessage());
        }
    }
}
