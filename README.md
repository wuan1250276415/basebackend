...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Modules-30%2B-blue?style=flat-square" />
...
> 文档分工说明：
> - `wiki/`：长期使用说明、模块介绍、快速上手、架构导览（与 GitHub Wiki 对应）
> - `docs/`：项目内参考资料、规格说明、治理审查、阶段报告、知识库
...
```
basebackend/                                    当前约 30 个 Maven Reactor 模块 + 多个前端/业务目录
│
├── 🚀 微服务 (可部署)
...
> 说明：以下统计曾用于阶段性盘点，随着模块治理和结构清理推进，旧数字已不再精确。
> 当前更可信的口径是：**约 30 个 Maven Reactor 模块**，并包含多个前端与业务目录。
>
> 如需精确统计，建议以 `pom.xml`、源码扫描脚本和 CI 结果为准，而不是手工维护 README 常量。
...
- **GitHub Wiki / `wiki/`**：长期使用说明、模块介绍、快速启动、架构导览
- **`docs/reference/`**：项目内参考资料、接口盘点、工具说明
- **`docs/specs/`**：系统规格说明、专题设计文档
- **`docs/review/`**：结构治理、模块盘点、审查报告
- **`docs/reports/`**：阶段性执行报告、修复总结、状态汇报
- **`docs/knowledge-base/`**：专题知识库
