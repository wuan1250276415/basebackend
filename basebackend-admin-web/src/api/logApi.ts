...
export const logApi = {
  ...
  loginLogPage: (params: {
    ...
    request.get('/api/system/logs/login', { params }),
  ...
  operationLogPage: (params: {
    ...
    request.get('/api/system/logs/operation', { params }),

  /** 获取操作日志详情 */
  logDetail: (id: string): Promise<OperationLogDTO> =>
    request.get(`/api/system/logs/operation/${id}`),

  /** 删除操作日志 */
  deleteOperationLog: (id: string): Promise<void> =>
    request.delete(`/api/system/logs/operation/${id}`),

  /** 批量删除操作日志 */
  batchDeleteOperationLog: (ids: string[]): Promise<void> =>
    request.delete('/api/system/logs/operation/batch', { data: ids }),

  /** 清空操作日志 */
  cleanOperationLog: (): Promise<void> =>
    request.delete('/api/system/logs/operation/clean'),

  /** 批量删除登录日志 */
  batchDeleteLoginLog: (ids: string[]): Promise<void> =>
    request.delete('/api/system/logs/login/batch', { data: ids }),
