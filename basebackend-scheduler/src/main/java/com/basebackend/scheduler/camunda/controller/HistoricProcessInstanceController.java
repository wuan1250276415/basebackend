...
public class HistoricProcessInstanceController {
    ...
    @Operation(
        summary = "历史流程实例分页查询",
        description = "支持租户、业务键、流程定义、启动人和完成状态过滤的分页查询"
    )
    @GetMapping
    public Result<PageResult<HistoricProcessInstanceDTO>> page(@ParameterObject @Valid ProcessInstanceHistoryQuery query) {
    ...
    @Operation(
        summary = "获取历史流程实例详情",
        description = "查看历史流程实例详情，包含流程变量与活动历史轨迹"
    )
    ...
    @Operation(
        summary = "查询历史流程实例状态",
        description = "查询历史流程实例的完成、未完成、终止状态"
    )
     ...
     * 查询历史流程实例活动历史
     *
     * <p>使用精简的分页查询对象，只接收 current 和 size 参数。
     ...
     */
    @Operation(
        summary = "查询历史流程实例活动历史",
        description = "分页查询指定历史流程实例的活动执行轨迹，仅需分页参数（current/size）。\n\n"
            + "**请求示例**：\n"
     ...
     */
    @Operation(
        summary = "查询历史流程实例审计日志",
        description = "查询指定历史流程实例的用户操作审计日志，仅需分页参数（current/size）。\n\n"
            + "**请求示例**：\n"
