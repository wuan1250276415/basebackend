...
public record UpdatePreferenceDTO(
    ...
    @Schema(description = "菜单收起状态（0-展开，1-收起）") Integer menuCollapse,
    ...
    // 通知偏好
    @Schema(description = "是否启用邮件通知（0-否，1-是）") Integer emailNotification,
    @Schema(description = "是否启用短信通知（0-否，1-是）") Integer smsNotification,
    @Schema(description = "是否启用系统通知（0-否，1-是）") Integer systemNotification,

    // 其他偏好
    @Schema(description = "分页大小") Integer pageSize,
    @Schema(description = "仪表板布局配置（JSON格式）") String dashboardLayout,
    @Schema(description = "自动保存（0-否，1-是）") Integer autoSave
) {}
