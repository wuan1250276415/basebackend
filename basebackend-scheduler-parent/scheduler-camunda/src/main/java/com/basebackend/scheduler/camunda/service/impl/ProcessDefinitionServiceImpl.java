...
public class ProcessDefinitionServiceImpl implements ProcessDefinitionService {
    ...
    public BinaryPayload downloadBpmn(String processDefinitionId) {
        try {
            log.info("Downloading BPMN resource, definitionId={}", processDefinitionId);
            ...
            if (definition == null) {
                throw new CamundaServiceException("流程定义不存在: " + processDefinitionId);
            }
            ...
            return BinaryPayload.builder().data(content).fileName(fileName).mimeType("application/xml").build();
        } catch (CamundaServiceException ex) {
            throw ex;
        } catch (Exception ex) {
            log.error("Failed to download BPMN resource, definitionId={}", processDefinitionId, ex);
    ...
    public BinaryPayload downloadDiagram(String processDefinitionId) {
        try {
            log.info("Getting process diagram, definitionId={}", processDefinitionId);
            ...
            return BinaryPayload.builder().data(content).fileName(fileName).mimeType(contentType).build();
        } catch (CamundaServiceException ex) {
            throw ex;
        } catch (Exception ex) {
            log.error("Failed to get process diagram, definitionId={}", processDefinitionId, ex);
