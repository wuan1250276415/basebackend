...
public class SqlInjectionPreventionInterceptor implements InnerInterceptor {
    ...
    private String getMapperId(StatementHandler sh) {
        try {
            ...
            if (delegate != null) {
                ...
                MappedStatement ms = (MappedStatement) delegateMeta.getValue("mappedStatement");
                if (ms != null) {
                    return ms.getId();
                }
            }
        } catch (Exception e) {
            log.debug("Failed to get mapper ID", e);
        }
        return "unknown";
    }
    ...
    public static java.util.Map<String, Object> getStatistics() {
        ...
        stats.put("blockRate", TOTAL_CHECKS.get() > 0 ? 
            (double) BLOCKED_COUNT.get() / TOTAL_CHECKS.get() * 100 : 0);
