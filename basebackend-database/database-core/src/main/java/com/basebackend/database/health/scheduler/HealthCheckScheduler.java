...
public class HealthCheckScheduler {
    ...
    /**
     * 故障转移集成（可选，由 database-failover 模块提供实现）
     */
    private DatabaseFailoverIntegration failoverIntegration;
    ...
    /**
     * 注入故障转移集成（可选，由 database-failover 模块提供）
     */
    @Autowired(required = false)
    public void setFailoverIntegration(DatabaseFailoverIntegration failoverIntegration) {
        this.failoverIntegration = failoverIntegration;
        log.info("Failover integration registered with health check scheduler");
    }
    ...
    public void performHealthCheck() {
        ...
        try {
            ...
            if (health.getStatus() != DataSourceHealth.HealthStatus.UP) {
                ...
                if (failoverIntegration != null && properties.getFailover().isEnabled()) {
                    log.warn("Master database is unhealthy, triggering failover handler");
                    failoverIntegration.handleMasterFailure(dataSource);
                }
            }
    ...
    public void performRecoveryCheck() {
        ...
        if (failoverIntegration == null) {
            return;
        }

        try {
            log.debug("Starting scheduled recovery check");
            failoverIntegration.performRecoveryCheck();
