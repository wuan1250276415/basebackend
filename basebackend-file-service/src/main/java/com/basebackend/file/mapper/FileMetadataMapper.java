...
public interface FileMetadataMapper extends BaseMapper<FileMetadata> {
     ...
     * 按存储类型聚合文件数量和大小
     */
    @Select("SELECT COALESCE(storage_type, 'LOCAL') AS storage_type, " +
            "COUNT(*) AS file_count, " +
     ...
     * 按文件扩展名聚合文件数量和大小
     */
    @Select("SELECT COALESCE(LOWER(file_extension), 'unknown') AS file_type, " +
            "COUNT(*) AS file_count, " +
            "COALESCE(SUM(file_size), 0) AS total_size " +
            "FROM file_metadata WHERE is_deleted = 0 " +
     ...
     * 查询文件总数和总大小
     */
    @Select("SELECT COUNT(*) AS total_files, " +
            "COALESCE(SUM(file_size), 0) AS total_size " +
            "FROM file_metadata WHERE is_deleted = 0")
