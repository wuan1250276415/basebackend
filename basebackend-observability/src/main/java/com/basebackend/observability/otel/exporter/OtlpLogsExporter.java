 ...
 * OTLP 日志导出器工厂
 * <p>
 * 创建配置好的 gRPC 日志导出器，用于将日志数据发送到支持 OTLP 协议的后端。
 * </p>
 *
...
public class OtlpLogsExporter {

    private static final Logger log = LoggerFactory.getLogger(OtlpLogsExporter.class);

    private final OtelProperties properties;

    public OtlpLogsExporter(OtelProperties properties) {
    ...
    public LogRecordExporter create() {
        ...
        log.info("正在配置 OTLP 日志导出器，端点: {}", endpoint);
